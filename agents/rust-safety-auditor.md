...
9. **Flawed Atomic Ordering** (`atomic-ordering-pair`): Reject causal data synchronization using `Ordering::Relaxed` or unjustified `Ordering::SeqCst`. Enforce `Release`/`Acquire` pairs.
...
- [ ] Are atomic operations using sound memory orderings (`Release`/`Acquire`)?
