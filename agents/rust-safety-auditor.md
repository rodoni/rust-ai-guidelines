...
5. **Untyped or Leaky Errors in Libraries** (`m-errors-canonical`): Reject opaque `String`, `Box<dyn Error>`, or `anyhow` errors in public library APIs. Require canonical error structs with backtraces and semantic query methods; avoid leaking internal error variants or dependencies.
